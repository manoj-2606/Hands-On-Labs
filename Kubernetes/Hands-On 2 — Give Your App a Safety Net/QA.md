...
When you do a rolling update, the Deployment creates a new ReplicaSet (new version) and scales it up while scaling the old one down. The old ReplicaSet is kept at 0 for rollback. You can't do this with two layers — you'd lose rollback history.
...
| | CPU | Memory |
|---|---|---|
| `requests` | Guaranteed allocation — scheduler places Pod on a node with enough free CPU | Guaranteed allocation — scheduler ensures node has this much free memory |
